...
def download_bids_for_runlevel(gtk_context, hierarchy, 
                               tree=False, tree_title=None,
                               src_data=False, folders=[], dry_run=False):
    ...
    if run_level == 'no_destination':
        ...
        if gtk_context.destination['type'] == 'analysis':
            ...
            log.info('Destination is acquisition, changing run_level to ' \
                     'acquisition')
            acquisition = gtk_context.client.get(gtk_context.destination['id'])
            hierarchy['acquisition_label'] = acquisition.label
            extra_tree_text += f'  {"acquisition_label":<18}: changed to ' + \
                               f'{acquisition.label}\n\n'
            run_level = 'acquisition'

        else:
            log.info('The destination "%s" is not an analysis or acquisition.',
        ...
        try:  # download BIDS data for the proper run level
            ...
            if run_level == 'project':
                ...
                log.info('Downloading BIDS for project "%s"',
                ...
                log.info('Downloading BIDS for subject "%s"',
                         hierarchy['subject_code'])
                ...
                log.info('Downloading BIDS for session "%s"',
                ...
                if hierarchy['acquisition_label'] == 'unknown acqusition':
                    msg = 'Cannot download BIDS for acquisition "' + \
                    ...
                    log.info('Downloading BIDS for acquisition "%s"',
                             hierarchy['acquisition_label'])

                    try:
                        # only download acquisition data
                        bids_path = Path(gtk_context.work_dir) / 'bids'
                        download_bids_dir(gtk_context.client,
                                          gtk_context.destination['id'],
                                          'acquisition',
                                          bids_path, src_data=src_data,
                ...
                msg = 'This job is not being run at the project, subject, ' +\
    ...
    if tree:
        tree_bids(bids_path, str(Path(gtk_context.output_dir) / 'bids_tree'),
                  tree_title, extra_tree_text)
